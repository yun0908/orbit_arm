...
class dxlControl_X(object):
    ...
    def move2goal(self, goalPoss):
        ...
        :type goalPoss: np.ndarray

        """
        global DATA_C, DATA_P, DATA_V

        self.enable_torque(self.IDs)
        assert isinstance(goalPoss, np.ndarray)
        for id in self.IDs:
    ...
    def radarr2Pos(goalPos):
        """弧度转换为电机位置"""
        assert isinstance(goalPos, np.ndarray)
        return goalPos / (np.pi * 2) * 4096 + 2048
...
if __name__ == '__main__':
    ...
    d.move2goal(np.array([2048, 2048, 2048, 2048, 2048, 2048]))
    d.disable_torque()
