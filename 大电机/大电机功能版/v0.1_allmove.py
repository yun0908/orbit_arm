...
read_data_speed1 = ["18"]    #读取正向速度
read_data_speed2 = ["19"]   #读取反向速度
send_data_speedcontrol1 = ["24", "f3", "01", "00", "00"]  #设置正向速度
send_data_speedcontrol2 = ["25", "9b", "97", "99", "19"]  #设置反向速度

len_send_stop = len(send_data_disable) #1字节
len_send_move = len(send_data_POS1)    #5字节
...
#设置速度，初始速度缩小10倍
for i in range(len_send_ID):
    sendcan1(ID_CAN1[i], len_send_move, send_data_speedcontrol1)
    sendcan1(ID_CAN1[i], len_send_move, send_data_speedcontrol2)

# Move
for j in range(len_send_ID):

    sendcan1(ID_CAN1[j], len_send_stop, read_data_speed1)    #读取正向速度
    sendcan1(ID_CAN1[j], len_send_stop, read_data_speed2)    #读取反向速度
    sendcan1(ID_CAN1[j], len_send_move, send_data_POS2)      #转到位置90
