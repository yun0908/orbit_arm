...
#  读取位置 --> 角度制         两电机 范围 0-->180  以3600为一圈基准，1800为0位   1800 == 90du   1du == 20
#  初始构型 pos1
def ReadPosition():
    global musbcanopen, rec_CAN1
    readpositions = []
    read = [] #存十进制数
    res = 0
    if musbcanopen == True:
        ...
        while (scount < 3):
            ...
            if len > 0 and ret == 1:
                for i in range(4, rec[0].DataLen):
                    read.append(int(rec[0].data[i]))
                res = read[0] + (read[1] << 8) + (read[2] << 16) + (read[3] << 24)
            position = (int(res) / 6553600) * 3600   #扩大十倍
            readpositions.append(position)
        # t = threading.Timer(0.03, ReadPosition)
        # t.start()
        print("十进制初始位置：", readpositions)
    return readpositions  #十进制数


#输入五字节指令 --> 十进制/十六进制
#目标构型 pos2
def WritePosition(send_data_POS):
    write = []
    for i in range(1, 5):
        write.append(int(send_data_POS[i]))
    res = write[0] + (write[1] << 8) + (write[2] << 16) + (write[3] << 24)
    position = (int(res) / 6553600) * 3600       #扩大十倍
    print("十进制目标位置：", position)
    return position     #十进制数


# 分割函数（速度控制函数） 将两个姿态分割为n份
# n为想要分割的数量, pos2为目标构型，pos1为起始构型
def SpeedControl(pos1, pos2, n):
...
def sendcan1(ID_CAN1, len_sdata, send_data):
    global musbcanopen  # sh声明全局变量
    if (musbcanopen == False):
        print("ERROR, 请先打开设备")
    else:
        information = send_data  # 电机指令
        canobj = CAN_OBJ()
...
'''
    白电机 指令集
'''
send_data_POS0 = ["44", "00", "00", "00", "00"]
send_data_POS1 = ["44", "00", "60", "00", "00"]  # position1  1.35度
send_data_POS2 = ["44", "00", "40", "19", "00"]  # position2  90.9度
send_data_POS180 = ["44", "00", "00", "32", "00"]
send_data_POS360 = ["44", "00", "00", "64", "00"]
...
send_data_speed3 = ["1d", "e8", "20", "00", "00"]  # v3    正转
send_data_disable = ["02"]  # 停止
send_data_position = ["41"]  # 获取电机当前位置
send_data_setid = ["2E", "00", "00", "00", "02"]  # 设置id
send_data_con_speed = ["24", "1c", "47", "00", "00"] #设置速度
write_data_con_speed = ["18"]

len_send_stop = len(send_data_disable)
len_send_move = len(send_data_POS1)
len_send_ID = len(ID_CAN1)
len_send_position = len(send_data_position)
...
#获取位置
for i in range(len_send_ID):
    sendcan1(ID_CAN1[i], len_send_position, send_data_position)
    ecan.ClearBuffer(USBCAN2, DevIndex, Channel1)
    print("haha")


# #Move single
# sendcan1(ID_CAN1[3], len_send_move, send_data_POS360) #修改ID_CAN1[]
# sendcan1(ID_CAN1[3], len_send_stop, write_data_con_speed)
# time.sleep(0.5)
# # ReadCAN()
# ecan.ClearBuffer(USBCAN2, DevIndex, Channel1)
# print("hah")
...
# print(y)
# print("------------------------------------------")
# x = WritePosition(send_data_POS360)   #正常
# print(x)
# print("------------------------------------------------")
# SpeedControl(y, x, 3)       #无线循环
# print("###################################################")
# #测试ID
# for i in range(127):
#     id = str(i+1)
#     # print(type(id))
#     print(id)
#     sendcan1(id, len_send_stop, send_data_disable)
#     ReadCAN()
...
# for j in range(len_send_ID):
#     sendcan1(ID_CAN1[j], len_send_move, send_data_POS0)
#     time.sleep(0.5)
#     # ReadCAN()
#     ecan.ClearBuffer(USBCAN2, DevIndex, Channel1)
#     print("hah")
...
# #Move single
# sendcan1(ID_CAN1[0], len_send_move, send_data_POS0)     #修改ID_CAN1[]
# time.sleep(0.5)
# # ReadCAN()
# ecan.ClearBuffer(USBCAN2, DevIndex, Channel1)
# print("hah")
